...
Una aplicación full-stack de autenticación construida con MongoDB, Express, React y Node.js, que incluye capacidades avanzadas de autenticación de usuarios y una utilidad Git Diff Tracker para la gestión del flujo de trabajo de desarrollo.
...
### Git Diff Tracker
- **Preservación automática** de git diffs (sin intervención manual necesaria)
- **Git hooks** guardan diffs en cada commit
- **Vigilante en segundo plano** guarda automáticamente cada 5 minutos
- Acceso CLI y API REST al historial de diffs
- Limpieza automática de diffs antiguos
- Comparación de cambios entre diferentes puntos en el tiempo
- Exportación del historial de diffs a JSON
...
├── .gitignore
├── README.md
└── GIT_DIFF_TRACKER.md           # Documentación detallada del git diff tracker
```
...
# Opcional: Configurar vigilante de diffs (valores predeterminados mostrados)
...
**¡El Git Diff Tracker se iniciará automáticamente y preservará tus cambios!**

## Inicio Rápido - Preservación de Diffs

Una vez que el servidor esté en ejecución, tus git diffs se preservan automáticamente mediante:

1. **Vigilante automático** - Guarda cada 5 minutos
...
Obtener el git diff actual (cambios en staging y sin staging)
...
```json
{
  ...
  "data": {
    ...
    "branch": "main",
    "lastCommit": "abc1234 Mensaje de commit",
    "stagedDiff": "...",
    "unstagedDiff": "...",
    "combinedDiff": "..."
  }
...
Guardar el git diff actual en el historial
...
Comparar diff actual con el diff guardado anterior
...
## CLI del Git Diff Tracker

El Git Diff Tracker incluye una interfaz de línea de comandos para acceso rápido al historial de diffs.
...
También puedes usar el Git Diff Tracker programáticamente en tu código Node.js:
...
- Los JWTs se usan para la gestión de sesiones
...
4. Usa el git diff tracker para preservar el historial de cambios durante el desarrollo
...
## Beneficios del Git Diff Tracker
...
### Problemas del Git Diff Tracker
...
- El tracker automáticamente mantiene solo los últimos 50
...
- **[DIFF_PRESERVATION_GUIDE.md](./DIFF_PRESERVATION_GUIDE.md)** - Guía completa para la preservación automática de diffs (¡empieza aquí!)
- **[GIT_DIFF_TRACKER.md](./GIT_DIFF_TRACKER.md)** - Referencia de API y detalles del Git Diff Tracker

## Contribuciones

Este proyecto es parte de un sistema de autenticación avanzado. Las contribuciones deben mantener los estándares de seguridad y seguir la estructura de código existente.
